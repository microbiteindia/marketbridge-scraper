...
async function getFlipkartProduct(pid) {
    ...
    console.time(`flipkart-total-${pid}`);
    ...
    try {
        ...
        console.time(`flipkart-goto-${pid}`);
        ...
        console.timeEnd(`flipkart-goto-${pid}`);

        console.time(`flipkart-extract-${pid}`);

        const result = await page.evaluate(async (targetPid) => {
            // Attempt 1: Fetch structured data from Flipkart API
            try {
                ...
                if (response.ok) {
                    ...
                    if (pageContext) {
                        ...
                        let image = pageContext.imageUrl || "";
                        if (!image && Array.isArray(pageContext.multimedia?.images)) {
                            image = pageContext.multimedia.images[0]?.url || "";
                        }
                        if (!image && Array.isArray(pageContext.media?.images)) {
                            image = pageContext.media.images[0]?.url || "";
                        }
                        if (!image && Array.isArray(pageContext.productImages)) {
                            image = pageContext.productImages[0]?.url || "";
                        }
        ...
        console.timeEnd(`flipkart-extract-${pid}`);
        console.timeEnd(`flipkart-total-${pid}`);
