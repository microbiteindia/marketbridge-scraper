...
async function searchAmazon(keyword) {
	...
	page = await browser.newPage();
        ...
        // ---------------------------------------------------------
        // 3. Amazon normal search
        // ---------------------------------------------------------

        const searchKeyword = keyword.trim();

        const targetUrl =
            `https://www.amazon.in/s?k=${encodeURIComponent(searchKeyword)}`;
        ...
        const detectedBrand = await page.evaluate((keyword) => {
            ...
            // -----------------------------------------------------
            // Find Amazon Brand filter links
            // -----------------------------------------------------
            ...
            for (const link of links) {
                ...
                // Amazon brand-filter URL indicators.
                if (
                    !hrefLower.includes("p_89") &&
                    !hrefLower.includes("brand") &&
                    !hrefLower.includes("rh=")
                ) {
                    continue;
                }
                ...
                // -------------------------------------------------
                // Avoid obvious non-brand filter labels
                // -------------------------------------------------
            ...
            for (const candidate of brandCandidates) {
                ...
                // Does the search itself contain this brand?
                ...
                // Count result titles containing the brand.
                ...
                // Strongest signal:
                // brand explicitly appears in search.
                if (queryMatch) {
                    score += 100;
                }
                ...
                // Strong signal:
                // brand appears in many result titles.
            ...
            // For inferred brands, require at least:
            //
            // - 3 matching results
            // - 40% of results
            //
        ...
        if (detectedBrand) {
            ...
            const brandData =
                JSON.parse(detectedBrand);


            const absoluteBrandUrl =
                new URL(
                    brandData.href,
                    "https://www.amazon.in"
                ).href;
	...
	const products = await page.evaluate((searchTokens) => {
            ...
            const extractPrice = (card) => {
                ...
                const activePriceEl =
                    card.querySelector(
                        '.a-price:not([data-a-stripe="true"]):not(.a-text-price)'
                    );
...
const currentRank = nativeRank++;
                ...
                const ratingElement =
                    el.querySelector(
                        "i.a-icon-star-small span, i.a-icon-star span, .a-icon-alt"
                    );
